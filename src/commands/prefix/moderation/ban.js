...
module.exports = {
  ...
  run              : async (message, language, [member = '', ...reason]) => {
    ...
    if (message.client.owners.includes(member.id)){
      return message.reply(language.get({ '$in': 'COMMANDS', id: 'BAN_IS_HUTAODEV'  , parameters }));
    };
    ...
    return member.ban({ reason: `Hu Tao Ban Command: ${message.author.tag}: ${reason}`, days: deletemsgs ? 7 : 0 })
